 ...
 *
 * <p>
 * Span buffer methods that take positional arguments tend to come int 2 flavors. One takes absolute position the other
 * takes relative position.
 * </p>
 *
 * <p>
 * Absolute position is defined as buffer.getOffset()+relativePosition. Absolute position has a range of
 * (getOffset(),getOffset()+getLength())
 * </p>
 *
 * <p>
 * Relative position is the position within the buffer when visualized as a byte buffer. Relative position has a range
 * of (0,getLength())
 * </p>
 */
public interface SpanBuffer extends Span {

    /**
     * Return the relative version of the absolute value. Where the absolute position may be outside the range of the
     * span. Negative value may be returned from this method.
     *
     * @param absolute
     *            the position to convert to relative.
     ...
     *
     * @param relative
     *            the relative postion to convert to absolute.
    ...
    /**
     * When the candidate is greater than the relative upper limit the upper limit is returned.
     *
     * @param candidate
    ...
    /**
     * When the candidate is greater than the absolute upper limit the upper limit is returned.
     *
     * @param candidate
     *            the candidate to check.
     * @return the maximum value or the candidate if it is in range.
    ...
    /**
     * When the candidate is smaller than zero then zero is returned, otherwise the candidate is returned.
     *
     * @param candidate
     *            the candidate to check.
    ...
    /**
     * When the candidate is smaller than the absolute lower limit then the absolute lower limit is returned, otherwise
    ...
    /**
     * Create a duplicate of this buffer. This is used in cases where we need to create a new span buffer with differing
     * offset.
     *
     * @param newOffset
     *            new offset
     ...
     *
     * <p>
     * The offset of the new span will be position. The byte at position in the original buffer will the the first byte
     * in the resulting buffer. The rest of the original buffer will be in the resulting buffer.
     *
     * </p>
     * <p>
     ...
     *
     * @param position
     *            the absolute position to slice at.
     * @return A slice from the original buffer.
     * @throws IllegalArgumentException
     *             if the position is not within the span.
     * @see #cut
     * @see #getOffset
     * @see #getLength
     */
    ...
    /**
     * Like sliceAt() except the that byteCount is the number of bytes to cut off the front.
     *
     * <p>
     * See discussion of Absolute and Relative methods above.
     *
     ...
     *
     * @param byteCount
     *            the number of bytes to cut off the front.
     * @return the new buffer comprising the old buffer with byteCount bytes removed from the front.
     * @see #sliceAt
     */
    SpanBuffer cut(long byteCount);

    /**
     * Create a new SpanBuffer by truncating at the specific location. The location will be included as the last byte in
     * the new buffer.
     *
     * <p>
     * See discussion of Absolute and Relative methods above.
     ...
     *
     * @param position
     *            The absolute last position to include in the new buffer.
    ...
    /**
     * Create a new SpanBuffer taking the last byteCount bytes from the current buffer.
     *
     * <p>
     * the offset of the new span will be the offset of the position specified by byteCount as determined from the start
     * position of the current buffer.
     * </p>
     *
     * @param byteCount
     *            the number of bytes to take counting from the end of the buffer.
     * @return A slice from the original buffer.
     * @throws IllegalArgumentException
     *             if the resulting position is not within the span.
     */
    SpanBuffer tail(long byteCount);

    /**
     * Create a new SpanBuffer taking the first byteCount bytes from the current buffer.
     *
     * <p>
     * See discussion of Absolute and Relative methods above.
     *
    ...
    /**
     * Create a new SpanBuffer by appending the contents of otherBuffer to the end of this buffer.
     *
     * @param otherBuffer
     *            the buffer to concatenate to the end of this one.
     * @return the new span buffer.
     */
    SpanBuffer concat(SpanBuffer otherBuffer);

    /**
     * Like tail() create a new SpanBuffer by slicing at a specific position from the end of the buffer the offset of
     * the new span will be the offset of the position. The difference is that safeTail() will not allow l to move
     * before the start of the buffer.
     *
     * @param position
     *            the position from the end to slice at.
     * @return A slice from the original buffer.
     */
    SpanBuffer safeTail(long position);

    /**
     * Like sliceAt() except that if the position is outside the span and empty SpanBuffer is returned.
     *
     * @param position
     *            the position to slice at.
     * @return A slice from the original buffer.
     */
     ...
     * @param loc
     *            The absolute position to start searching from.
    ...
    /**
     * Get the last absolute position of the needle in the SpanBuffer starting at specified absolute position. The
     * result is a relative value as well.
     *
     * <p>
     ...
     *            The needle to search for.
     * @param pos
     *            The absolute position start searching from.
     * @return The last relative position of the needle in the SpanBuffer before pos.
     * @throws NoMatchException
     *             if the needle is not found
     */
    ...
    /**
     * Get the last absolute position of the needle in the SpanBuffer. Result is relative to getOffset()
     *
     * <p>
     ...
     *
     * @param limit
     *            the maximum number of bytes to encode.
     * @return the contents of the buffer as a hex encoded string.
     * @throws IOException
     *             on error.
     */
    ...
    /**
     * Read the bytes from the specified absolute position into the provided buffer. Equivalent to calling <code>read(
     ...
     *
     * @param position
     *            the absolute position to start read from.
     * @param buff
     ...
     *            the buffer to copy the bytes into.
     * @param pos
     *            the position to copy the bytes into.
     * @param len
     ...
     *
     * @param byteOffset
     *            the relative position to read from.
     * @return the byte
     * @throws IOException
     *             on error
     * @see #read(long)
    ...
    /**
     * Read the bytes from the specified relative position into the provided buffer. Equivalent to calling readRelative(
     * position, buff, 0, buff.length );
     *
     * <p>
     * This is an relative method.
     *
     ...
     *
     * @param byteOffset
     *            the relative position to begin read from.
     * @param buff
     *            the buffer to copy the bytes into.
     * @return the number of bytes read. May be less then length
     * @see #read(long, byte[])
     ...
     *
     * @param position
     *            The absolute starting position within the buffer.
     ...
     * @param other
     *            the other buffer.
     * @return true if the bytes at the start of this buffer are the same as from the other buffer.
     * @throws IOException
     *             on error
     */
     ...
     * @param other
     *            the other buffer.
     * @return true if the bytes at the end of this buffer are the same as from the other buffer.
     * @throws IOException
     *             on error
     */
    ...
    interface Walker {
         ...
         *             if the numberOfBytes moves past the end of the buffer
         */
         ...
         *
         * @param position
         *            the absolute position to set to.
         * @throws NoSuchElementException
    ...
    /**
     * Class that contains reference implementations for various span buffer methods.
     */
    class Utils {
        /**
         * Default equality. Buffers are equal if they are the same length and have the same bytes in the same order.
         * offset is not considered.
         *
         * @param one
         *            the first span buffer
         * @param two
         *            the second span buffer
         ...
         *
         * @param one
         *            the span buffer to bulid the hash code for.
         ...
         * @throws IOException
         *             on IO error or if buffer length is greater than Integer.MAX_VALUE.
         */
        public static byte[] asByteBuffer(final SpanBuffer sb) throws IOException {
            if (sb.getLength() > Integer.MAX_VALUE)
            {
                throw new IOException( String.format( "Buffer length (%s) is greater thatn Integer.MAX_VALUE (%s)", sb
                        .getLength(), Integer.MAX_VALUE ) );
            }
         ...
         *
         * @param sb
         *            the span buffer to get simple text for.
         * @return the string.
         */
        public static String simpleText(final SpanBuffer sb) {
            try
            {
                return String.format( "%s %s", Span.toString( sb ), sb.getHex( 10 ) );
            } catch (final IOException ex)
            {
                return String.format( "%s %s", Span.toString( sb ), "unable to retrieve" );
            }
