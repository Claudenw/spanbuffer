...
public class MatcherImpl implements Matcher {
        ...
        try
        {
            for (myPatternPos = 1; myPatternPos <= myPatternLen; myPatternPos++)
            {
                ...
                for (myTargetPos = 1; myTargetPos <= myTargetLen; myTargetPos++)
                {
                    ...
                    // left and up +cost
                    costAry[myTargetPos] =
                            Math.min( Math.min( costAry[myTargetPos - 1] + 1, prevCostAry[myTargetPos] + 1 ),
    ...
    public int getLevenshteinDistance(final SpanBuffer pattern, final int threshold) {
         ...
         * *|#|#|#| We must ignore the entry to the left of the leftmost member We must ignore the entry above the
         * rightmost member
         *
        ...
        try
        {
            ...
            for (int j = 1; j <= myPatternLen; j++)
            {
                ...
                final int max =
                        (j > (Integer.MAX_VALUE - threshold)) ? myTargetLen : Math.min( myTargetLen, j + threshold );
    ...
    public Result bitap(final SpanBuffer pattern, final long loc) throws IOException {
        ...
        {
            /*
             * Scan for the best match; each iteration allows for one more error. Run a binary search to determine how
             * far from 'loc' we can stray at this error level.
             */
            binMin = 0;
            binMid = binMax;
            while (binMin < binMid)
    ...
    /**
     * Compute and return the score for a match with errCnt errors and matchPos location.
     *
     * @param errCnt
