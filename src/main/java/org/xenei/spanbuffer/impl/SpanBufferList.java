...
/**
 * A SpanBufferList is an SpanBuffer which comprises of multiple SpanBuffer objects. This allows creation of SpanBuffers
 * that take bits and pieces from other SpanBuffers and make them all act as one SpanBuffer.
 */
public class SpanBufferList extends AbstractSpanBuffer {
    ...
    /**
     * Constructor. Create a SpanBufferList with an offset specified by the offset parameter. The SpanBuffers listed in
     * the spans will be concatenated to create one SpanBuffer starting from offset and having a length equal to the
     * total length of the buffers in the spans argument.
     *
     * @param offset
     *            the offset for the first byte of this SpanBuffer.
     * @param spans
     *            An array of SpanBuffer that will be assembled into a single coherent buffer.
     */
     ...
     * @param position
     *            The position to locate.
     * @return The index of the SpanBuffer containing the position or -1 if not found.
     */
