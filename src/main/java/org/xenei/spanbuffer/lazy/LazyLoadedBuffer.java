 ...
 *
 * <p>
 * Because new instances of LazyLoadBuffer that wrap the same LazyLoader instance are returned from methods like
 * duplicate(), head(), sliceAt(), etc. LazyLoadBuffer maintains some additional internal state that isn't required
 * for other implementations like 
 * <ul>
 * <li>inset - a position analogous to the position property in ByteBuffer (used internally by
 * SpanByteBuffer)</li>
...
public class LazyLoadedBuffer extends AbstractSpanBuffer {
    /**
     * The position within the lazy loaded buffer that is our offset in the buffer. Compare to offset - the offset of
     * the buffer with respect to an outside index.
     */
    ...
    /**
     * if lazyLoader doesn't have a prepopulated length, construction of SpanLazyLoadBuffer will cause the lazyLoader to
     * load its data.
     *
     * @param offset
     *            The external offset for this buffer.
     * @param lazyLoader
     *            the lazy loader.
     */
     ...
     *            The external offset for this buffer.
     * @param inset
     *            The offset into the internal buffer where this buffer starts.
     * @param lazyLoader
     *            the lazy loader.
     */
     ...
     *            The offset into the internal buffer where this buffer starts.
     * @param bufferLength
     *            the length of the internal buffer or UNDEF_LEN if unknown.
     * @param lazyLoader
     *            the lazy loader.
     */
    private LazyLoadedBuffer(final long offset, final int inset, final long bufferLength,
