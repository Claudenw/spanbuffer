 ...
 * 
 * Once the output stream is closed the position of the root node may be retrieved.  That position can
 * then be used with the Corresponding TreeDeserializer to create a TreeLazyLoader from which a Spanbuffer
 * may be obtained.
 * 
...
public class TreeOutputStream extends OutputStream {
    ...
    /* the stack of nodes we are writing to.  Leaf is node 0, the rest are inner nodes leading back
     * to the root node.
     */
    ...
    public void write(final byte[] buffer, final int off, final int len) throws IOException {
        ...
        try
        {
            ...
            } else
            {
                ...
                /* IF not, chop the data into a chunk that would have enough data to fit the leaf
                 node */
     ...
     *
     * @param buffer
     *            data buffer to be written in the node
     * @param nodeIndex
     *            index position in the stackNodeList
     * @param expandedLength
     *            the length of the actual data
     * @throws InterruptedException
     *             In case action was interrupted during execution
     ...
     *
     * @param nodeIndex
     *            Node position in the stackNodeList
     * @throws InterruptedException
     *             In case action was interrupted during execution
     * @throws ExecutionException
     *             Thrown at execution
    ...
    private void createRoot(int nodeIndex) throws InterruptedException, ExecutionException {
        ...
        if ((nodeIndex == LEAF_NODE_INDEX) && (stackNodeList.size() == 2)
                ...
                && stackNodeList.get( FIRST_INNER_INDEX )
                        .hasSpace( InnerNode.FLAG_SIZE ))
        {
    ...
    /**
     * Once we write the root we can retrieve the Position information about the node.
     * 
    ...
    /**
     * Writes the node's data to the KafkaQueue and retrieves the newly obtained data (OPLImpl).
     *
     * @param node
    ...
    /**
     * Writes the data for the root node using the serializer.  Updates the final position.
     *
     * @param rootNode
     *            The TreeNode that is the root node.
    ...
    protected void writeRoot(final TreeNode rootNode) throws ExecutionException, InterruptedException {
        ...
        if (TreeOutputStream.LOG.isDebugEnabled())
        {
            TreeOutputStream.LOG
                    .debug( "Obtained data for the Root Node, writing it" );
        }        
