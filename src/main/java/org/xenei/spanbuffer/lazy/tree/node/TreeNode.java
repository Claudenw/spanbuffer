...
public abstract class TreeNode {
     ...
     *
     * @param maxBufferSize
     *            This value must be over the minimum size (1 int, 2 long) * 2
     */
    public TreeNode(final int maxBufferSize) {
        span = Span.fromLength( 0, maxBufferSize );
     ...
     *
     * @param bytes
     *            the number of bytes required in the buffer.
    ...
    /**
     * Is the buffer empty?  This varies as inner nodes always have a byte written to the buffer
     * even if there is no 'real' data.  This will return true if there is no 'real' data.
     *
    ...
    /**
     * Writes data to the buffer.  The expanded length is the number of bytes represented by the data to store.
     * In leaf nodes this is the same as the dts length, for inner nodes the dts is probably an encoded Position
     * and may represent many more bytes when the position is expanded.  
     *
     * @param dts
     *            Data to store.
     * @param expandedLength
     *            length of the actual data represented  by the data to store.
     * @throws IllegalStateException if the data to store will not fit in the buffer.
     */
    ...
    /**
     * Adjust the length property node.  For leaf nodes this method does nothing.  For leaf inner nodes
     * it increments the length that the buffer represents.
     *
     * @param addedLength
     *            the number of bytes to add to the length.
     */
    protected abstract void adjustLength(long addedLength);

    /**
     * Clone the data from this node. This ensures that the byte array will not be modified by later 
    ...
    /**
     * Expanded length of the block.  For the leaf node this is the length of the buffer.  For inner nodes
     * this is the number of bytes encompassed by all the leaf nodes the inner nodes point to.
     *
