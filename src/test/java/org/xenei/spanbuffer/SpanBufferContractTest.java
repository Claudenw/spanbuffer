...
public class SpanBufferContractTest<T extends SpanBuffer> {
    ...
    public void testSliceAtPosition() {
        ...
        Assert.assertEquals( "Wong number of bytes read", span2.getLength(), span2.read( spanBuffer.getOffset(),
                buff ) );
        ...
        Assert.assertEquals( "Sliced end value is incorrect", Span.calcEnd( span2 ), span2.getEnd() );
        Assert.assertEquals( "Sliced length value is incorrect", Span.calcLength( span2 ), span2.getLength() );
    ...
    public void testPositionOfLoc() throws IOException, NoMatchException {
        ...
        Assert.assertEquals( "Should have found second instance", secondPos, spanBuffer.positionOf( span2, firstPos
                + 1 ) );
        Assert.assertEquals( "Should have found second instance", secondPos, spanBuffer.positionOf( span2,
                secondPos ) );
    ...
    private class LastDuplicateInfo {
        ...
        public LastDuplicateInfo() {
            /*
             * find a byte that is duplicated in the buffer retain first and second position of the byte and they byte
             * itself.
             */
            final Map<Byte, Integer> map = new HashMap<>();
            firstPos = -1;
    ...
    public void testLastPositionOfLoc() throws IOException, NoMatchException {
        ...
        Assert.assertEquals( "Should have found first instance", duplicateInfo.firstPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.firstPos ) );
        Assert.assertEquals( "Should have found first instance", duplicateInfo.firstPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.firstPos + 1 ) );
        Assert.assertEquals( "Should have found second instance", duplicateInfo.secondPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.secondPos ) );
        Assert.assertEquals( "Should have found second instance", duplicateInfo.secondPos, spanBuffer.lastPositionOf(
                span2, Math.min( duplicateInfo.secondPos + 1, spanBuffer.getEnd() ) ) );
    ...
    public void testLastPositionOfLocWithOffset() throws IOException, NoMatchException {
        ...
        {
            /*
             * find the last position of the byte where we start at the beginning. so no value is possible
             */
        ...
        /*
         * The last position starting from the firstPosition should be the first position.
         */
        Assert.assertEquals( "Should have found first instance", duplicateInfo.firstPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.firstPos ) );
        /*
         * The last position starting from the byte after the first position should be the first position
         */
        Assert.assertEquals( "Should have found first instance", duplicateInfo.firstPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.firstPos + 1 ) );
        /*
         * The last position starting from the second position should be the second position.
         */
        Assert.assertEquals( "Should have found second instance", duplicateInfo.secondPos, spanBuffer.lastPositionOf(
                span2, duplicateInfo.secondPos ) );
        /*
         * The last position starting from the second position + 1 should be the sedond position
         */
        Assert.assertEquals( "Should have found second instance", duplicateInfo.secondPos, spanBuffer.lastPositionOf(
                span2, Math.min( duplicateInfo.secondPos + 1, spanBuffer.getEnd() ) ) );
    ...
    public void testReadPositionWithOffset() throws IOException {
        ...
        {
            Assert.assertEquals( "buffer contents wrong", buffer[i], spanBuffer.read( i
                    + SpanBufferContractTest.TestOffset ) );
        }
    }
    ...
    public void testReadByteArrayWithExcessLength() {
        ...
        final int bytesRead = spanBuffer.read( 0, buff );
        Assert.assertEquals( "Should have read " + spanBuffer.getLength() + " bytes", spanBuffer.getLength(),
                bytesRead );
        for (int i = 0; i < spanBuffer.getLength(); i++)
        {
            Assert.assertEquals( "buffer contents wrong", buffer[i], buff[i] );
